...
export const LikeButton = ({ likes, blogId }: LikeButtonProps) => {
  ...
  return (
    <div className={`inline-flex items-center gap-2 ${!blogId || isSessionPending || !session ? "opacity-50" : ""}`}>
        ...
        <button onClick={blogId && session ? () => handleLike() : undefined} className={isPending || !blogId || !session || isSessionPending ? "cursor-not-allowed" : "cursor-pointer"} disabled={isPending || !blogId || !session || isSessionPending}>
